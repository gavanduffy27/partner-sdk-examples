...
public class ScannerDialogs {
  ...
  public static boolean promptContinue(String message, String title) {
    ...
    return isFalseString(next);
  }

  public static String promptDefault(String message, String defaultValue) {
    Scanner scanner = new Scanner(System.in);
    FormatUtils.print(message + ">> [" + defaultValue + "]");
    String response = scanner.nextLine();
    if (isNullString(response)) {
      response = defaultValue;
    }
    return response;
  }

  public static boolean promptYN(String message, boolean defaultValue) {
    String response = promptDefault(message, String.valueOf(defaultValue));
    return !isFalseString(response);
  }
