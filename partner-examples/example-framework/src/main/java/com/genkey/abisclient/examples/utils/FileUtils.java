...
/**
 * Various file utilities used to support the test examples. Implementation of these is of no
 * importance and they are provided for purpose of expedience only.
 *
 * <p>Worth noting however that this implements the use of global configuration directories using
 * the exact same mechanism as the product libraries, where the root configuration location on disk
 * is assumed to be ${GENKEY_HOME}\config where GENKEY_HOME is defined as a environment variable.
 *
 * @author gavan
 */
public class FileUtils {
  ...
  public static String mkFilePath(String dirName, String fileName, boolean checkPath)
      throws IOException {
    if (checkPath) {
      ;
  ...
  public static void createDirectory(String fileName, boolean tryCreatePath) throws IOException {
  ...
  public static String expandConfigPath(String pathName, String subPath) {
    return FileUtils.expandFilePath(
        getConfigurationPath(), FileUtils.mkFilePath(pathName, subPath));
  }
  ...
  public static String expandFileName(String pathName, String fileName, String ext) {
   ...
   * @throws IOException
   */
  public static void stringToFile(String text, String fileName, boolean append) throws IOException {
  ...
  /**
   * returns a list of files in specified directory that match the specified extension
   *
   * @param dirName The directory in which to look
   * @param ext Extension to filter by - null retrieves all files
   */
  ...
  public static String[] getMatchingFilenames(
      String dirName, String pattern, boolean expandName, boolean caseInsensitive) {
