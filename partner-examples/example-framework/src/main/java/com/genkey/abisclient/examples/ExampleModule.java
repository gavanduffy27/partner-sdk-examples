...
/**
 * Base class for example program modules that can be combined within an overall instance
 * ExampleSuite.
 *
 * @author gavan
 */
public abstract class ExampleModule extends GKTestCase4 implements IExampleTest {
  ...
  protected static final String SingleFingerDirectory =
      ImageSetPath("SingleFinger"); // "images/ABISClientBMP/SingleFinger";
  protected static final String TwoFingerDirectory =
      ImageSetPath("TwoFinger"); // "images/ABISClientBMP/TwoFinger";
  protected static final String FourFingerDirectory =
      ImageSetPath("FourFinger"); // "images/ABISClientBMP/FourFinger";
  ...
  public static class TestState {
    ...
    MatchEngineConfiguration.Settings matchEngineSettings =
        MatchEngineConfiguration.getInstance().getSettings();
  ...
  /**
   * Overrides the default message-handler to use a preferred implementation as the default for all
   * classes.
   *
   * <p>In general this is the only call required, however the handler can be specialised to be
   * different for different example modules, without affecting the global default.
   *
  ...
  protected abstract void runAllExamples();
  ...
  */
  ...
  public void doFeedback(QualityInfo qInfo, int index) {
    ...
    if (qInfo.getTilt() != 0) {
      if (qInfo.getTilt() < 0) {
        ...
        // top of the image.
        printMessage(
            "Or finger needs to be placed further forward so that tip is towards top edge of scanner");
      } else {
        ...
        printMessage(
            "Finger is too far forward beyond end of scanner, move finger so tip is within edge of scanner");
      }
    }
  ...
  /**
   * Static equivalent of PrintMessage which can be invoked statically to use the default handler.
   *
   * @param message
   */
