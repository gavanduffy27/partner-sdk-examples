...
/**
 * Mechanism to run mixed concurrency tests at the same time. Note this is not the same mechanism
 * used to enable the concurrency tests. It is more of a substitute for the JUNIT ParallelComputer
 * class that allows for different concurrency tests to be run at the same time.
 *
 * <p>Initialized by a set of ConcurrencyTest which are then separately run each in their own
 * thread. Note each test will then spawn its own threads independently with its own task-list.
 *
 * @author Gavan
 */
public class ParallelTestRunner {
  ...
  public void execute() {
    ...
    /**
     * This flag when set suppresses the ABISSystemReset call that would otherwise be called
     * multiple times during teardown phase
     */
