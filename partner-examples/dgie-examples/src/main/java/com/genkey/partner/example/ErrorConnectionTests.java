...
/**
 * These tests show-case how to test and handle errors that arise from failure in the connection between
 * client and server.
 * <p>
 * Much consideration has been given as to whether or not such issues should be handled as errors or thrown
 * as exceptions. The prevailing view is that the default behaviour should be to preferentially return error
 * status such as http response code as properties within response objects, which can be directly tested for
 * error states, or can be accessed in a thread safe manner from the service object.
 * <p>
 * It is however to be flexible and to adapt the behavior in accordance to the preferences of the application.
 * The examples below simulate the various error conditions, for example by using bad host names, and show 
 * how the application can manage errors for the different modes.
 * @author Gavan
 *
 */
public class ErrorConnectionTests extends PartnerExample {
	...
	public void setBadHostMode() {
		...
		// In fast fail we get the quickly failing BADHostName - otherwise it is the much slower connection timeout error
	 ...
	 * 
	 *  This mode observes the preference that when REST status is incorporated as part of the method return value
	 *  then no exception is thrown
	 *
	 *  However when the return method is of a simple type such as boolean then an exception will be thrown.
	 *  
	 *  These preferences are configurable as shown in subsequent examples.
	 */
	@Test
	public void testDefaultSuppression() {
		...
		RestServices.getInstance().setExceptionSuppressionMode(ExceptionSuppressionMode.SuppressDefault);
	...
	/**
	 * In this mode we set the exception handling to always throw.  This is maximum mode for throwing errors
	 * and it is shown in the example that in this case an exception will be thrown even though the response
	 * parameter implements the RestResponseParameter interface
	 */
	@Test
	 ...
	 * 
	 * For methods which do not return a value that implements RestResponseParameter we access this directly
	 * from the service object which also provides a thread safe implementation of the interface based on the
	 * status attributes of the last call on the current thread.
	 * 
	 */
	@Test
	public void testAlwaysSuppress() {
		...
		if ( !legacyService.isSuccess()) {
			// Process the error noting legacyService is thread safe implementation of RestResponseParameter
	...
	public void hostUnavailableTest() {
		...
		//Operate with default suppression - exceptions only get thrown for methods where the response does not contain a status code
		RestServices.getInstance().setExceptionSuppressionMode(ExceptionSuppressionMode.SuppressDefault);
		...
		/**
		 * Next call is on a web service interface that returns a boolean value
		 * For these calls the exception is always thrown because the result parameter does not provide access to
		...
		// Note the errors state of the service objects is based on ThreadLocal variables and this is thread safe
		...
		if (! legacyService.isSuccess()) {
			...
			if (errorCode != 200) {
				...
				printError("Legacy service failed with error " + errorCode + " and message " + errorMessage);
			}
		}
