...
public class DiagnosticExample extends PartnerExample {
  ...
  public void testImagePortraitPair() {
    ...
    ImagePortraitPair newPair =
        JAXBMarshallUtils.unmarshallNotStrict(ImagePortraitPair.class, xmlContent);
  ...
  public void testConfigAccess() {
    ...
    if (FileUtils.existsFile(testImageFile)) {
      printMessageF(
          "Image file %s for subject/finger/sample %d/%d/%d exists", testImageFile, 1, 1, 1);
      try {
      ...
      printErrorF(
          "Image file %s for subject/finger/sample %d/%d/%d not found ", testImageFile, 1, 1, 1);
      loadSuccess = false;
    }
    if (loadSuccess) {
    ...
    for (String resource : responseResources) {
      try {
        ...
        InspectResponse response =
            JAXBMarshallUtils.unmarshallXML2(InspectResponse.class, xmlContent);
