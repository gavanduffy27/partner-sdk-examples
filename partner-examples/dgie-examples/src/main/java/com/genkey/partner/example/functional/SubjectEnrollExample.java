...
public class SubjectEnrollExample extends FunctionalTestExample {
  ...
  /**
   * Repeat test that shows the creation of subjects, running a test and then resetting the system.
   * Note this is used only for the purpose of testing the Test Environment capacity to perform a
   * system reset.
   */
  @Test
  ...
  /**
   * Creates a number of subjects within the current test domain and on completion wipes all
   * subjects for that domain from the database.
   */
  @Test
  ...
  public void legacyVerify(String biographicId) {
    ...
    if (legacyService.existsSubject(biographicId)) {
      ...
      if (!verifyResponse.isVerified()) {
        super.printMessage(
            "Warning subject " + biographicId + " failed verification with score " + matchScore);
      } else {
        printMessage(
            "Subject " + biographicId + " verified against legacy with score " + matchScore);
      }
    }
  }
  ...
  public void faceFirstEnrolment(String biographicId) {}
  ...
  public void enquireStatusExample() {
    enquireStatusExample(EnrollmentSubjectID);
  }

  public void enquireStatusExample(String enrollmentSubjectID) {
    GenkeyABISService abisService = ABISServiceModule.getABISService();
    EnquireStatus status = abisService.enquireSubject(enrollmentSubjectID);
    printJsonObject("EnquireStatus", status);
  }
  ...
  public void biographicEnrollExample() {
    ...
    if (service.existsBiographicRecord(biographicId)) {
      String externalId =
          BiographicIdentifier.resolveExternalID(biographicId, service.getDomainName());
    ...
    BiographicProfileRecord biographicRecord =
        EnrollmentUtils.getSimpleBiographicRecord(biographicId, "john", "doe", Gender.Male);
  ...
  public void enrollDuplicateExample() {
    ...
    BiographicProfileRecord biographicRecord =
        EnrollmentUtils.getBiographicRecord(biographicId, "john", "Enroll");
    ...
    if (dupResponse.hasMatchResults()) {
      for (MatchResult result : dupResponse.getMatchResults()) {
        ...
        for (MatchResultDetail matchDetail : result.getMatchDetails()) {
          println(
              matchDetail.getIndex1()
                  + " <==>"
                  + matchDetail.getIndex2()
                  + ":"
                  + matchDetail.getScore());
        }
  ...
  public void identifyExample(String biographicId, int[] fingers) {
    identifyExample(biographicId, fingers, isFaceMatchEnabled());
  }
  ...
  public void identifyExample(String biographicId, int[] fingers, boolean faceEnabled) {
    ...
    String externalId =
        BiographicIdentifier.resolveExternalID(biographicId, abisService.getDomainName());

    checkSubjectEnrolled(biographicId);

    SubjectEnrollmentReference subject =
        EnrollmentUtils.enrollSubject(subjectNumber, fingers, 1, 1);
    if (faceEnabled) {
    ...
    if (!response.isSuccess()) {
      printMessage(
          "Handling REST response failure on query of "
  ...
  public void doVerifyExample(String biographicId, int[] fingers, boolean faceEnabled) {
    ...
    SubjectEnrollmentReference enrollmentRef =
        EnrollmentUtils.enrollSubject(subjectId, fingers, 1, 1);
    if (faceEnabled) {
    ...
    if (!verifyResponse.isSuccess()) {
      ...
      printMessage(
          "Verification failure with score " + verifyResponse.getMatchResult().getMatchScore());
      handleVerificationFailure(enrollmentRef, verifyResponse);
    } else {
      printMessage(
          "Verification success with score " + verifyResponse.getMatchResult().getMatchScore());
  ...
  private void doImageFetchExample(String biographicId, int[] fingers) {
    ...
    if (success) {
      ...
      for (int finger : wrapper.keySet()) {
        ...
        printResult(
            "Image " + finger,
            imageBlob.getImageEncoding().length + "/" + imageBlob.getImageFormat());

        // Obtain image in BMP format
        ImageData imageData =
            new ImageData(imageBlob.getImageEncoding(), imageBlob.getImageFormat(), resolution);
  ...
  public void standardEnrollExample(
    ...
    if (existsSubject) {
      EnquireStatus eStatus = abisService.enquireSubject(biographicId);
      if (eStatus.isFingersPresent()) {
        println("Subject " + externalId + " already exists. Processing as a verification test");
        doVerifyExample(biographicId, targetFingers);
        return;
      } else {
        println("Continuing insert since no fingers present");
      }
    }
    ...
    // BiographicProfileRecord biographicRecord =
    // EnrollmentUtils.getBiographicRecord(biographicId,
    // "john", "doe");
    boolean status = biographicService.insertBiographicRecord(biographicRecord);
  ...
  public void updateExample(String biographicId, int[] targetFingers, boolean faceMatchEnabled) {
    ...
    if (!existsSubject) {
      EnquireStatus eStatus = abisService.enquireSubject(biographicId);
      if (!eStatus.isFingersPresent()) {
        println("Subject exists but does not have fingers present");
      } else {
