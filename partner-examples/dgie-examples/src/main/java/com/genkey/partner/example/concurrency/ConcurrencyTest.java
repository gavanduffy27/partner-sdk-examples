...
public abstract class ConcurrencyTest extends PartnerExample {
  ...
  public void executeThreadTest(SubjectProcessorThread taskThread) {
    ...
    taskThread.initializeSubject(
        this.getStartSubject(), this.getSubjectCount(), this.getIterationCount(), 1);
  ...
  protected boolean checkSubjectPresent(
      long subject, int sampleIndex, String domainName, boolean status) {
    ...
    if (status) {
      if (!exists) {
        ...
        SubjectEnrollmentReference enrolRef =
            EnrollmentUtils.accessEnrollmentRecord(subject, sampleIndex, domainName);
    ...
    if (!service.existsSubject(externalId)) {
      ...
      SubjectEnrollmentReference enrolRef =
          EnrollmentUtils.accessEnrollmentRecord(
              subject, sampleIndex, domainName, this.cyclicShift);
  ...
  protected int deleteBiographicSubjects() {
    ...
    for (int ix = 0; ix < this.getSubjectCount(); ix++) {
      long subject = this.getStartSubject() + ix;
      String externalId =
          BiographicIdentifier.resolveExternalID(String.valueOf(subject), getDomainName());
  ...
  private boolean checkBiographicSubjectPresent(long subject, String domainName) {
    ...
    if (!service.existsBiographicRecord(externalId)) {
      ...
      BiographicProfileRecord biographicRecord =
          EnrollmentUtils.getBiographicRecord(
              externalId, "christian_" + subject, "surname_" + subject);
    ...
    void executeSubjectTask(SubjectEnrollmentReference ref, long subject, int sample) {
      ...
      if (exists) {
        ...
        BiographicProfileRecord record =
            EnrollmentUtils.getBiographicRecord(
                subjectId, "christian_" + subject, "surname_" + subject);
    ...
    void executeSubjectTask(SubjectEnrollmentReference enrolRef, long subject, int sample) {
      ...
      if (success) {
        ...
        println(
            subject
                + " is "
                + (response.isVerified() ? "verified" : "not verified")
                + " with score "
                + response.getMatchResult().getMatchScore());
      } else {
        String message = response.getErrorMessage();
        String result = response.getOperationResult();
        printResult("Operation results", result);
