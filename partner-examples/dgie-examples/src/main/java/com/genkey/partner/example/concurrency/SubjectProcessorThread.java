...
import com.genkey.partner.utils.concurrency.ConcurrentTaskThread;
 ...
 * <p>
 * The unit tasks are based on subject task queues specified by numeric-id.
 * <p>
 * The cyclic shift property in combination with the subject-id is used to construct the specific biometric
 * content for each processed task.
 * @author Gavan
 *
 */
public abstract class SubjectProcessorThread extends AbstractTestThread{
	...
	public SubjectProcessorThread(TaskController controller, List<Integer> samples, String domainName) {
	...
	protected void executeSubjectTask(long subject) {
		...
		for(int sample : this.getSamples()) {
			SubjectEnrollmentReference ref = EnrollmentUtils.accessEnrollmentRecord(subject, sample, this.getDomainName(), this.getCyclicShift());
			if (isSimulationMode()) {
	...
	private void simulateTask(long subject, int sample, String domainName) {
		FormatUtils.nl();
		FormatUtils.println(Commons.classShortName(this) + " simulated test for " + subject +"@" + domainName + "/" + sample);
	}
