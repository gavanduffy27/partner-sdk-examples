...
public class ABISUnknownSubjectExample extends BMSWorkshopExample {
  ...
  public void queryFaceExample(
      String biographicID, boolean insertIfNoMatch, boolean withBiographics) {
    ...
    if (withBiographics) {
      ...
      BiographicProfileRecord biographicRecord =
          EnrollmentUtils.getSimpleBiographicRecord(
              biographicID, "Shaka", "Dacaptain", Gender.Male);

      // Add the string encoded properties of the biographic record as matching context with
      // existing records
  ...
  private boolean checkAnyTrue(
      SubjectEnrollmentReference enrollRef, List<MatchResult> matchResults) {
  ...
  public void insertTestSubject(String biographicId, boolean withFace) {
    ...
    BiographicProfileRecord record =
        EnrollmentUtils.getBiographicRecord(String.valueOf(subjectId), "John", "Brown");
