...
public abstract class FunctionalTestExample extends PartnerExample {
  ...
  protected static void handleRESTFailure(
      SubjectEnrollmentReference subject, ABISResponse response, RemoteAccessService service) {
    handleRESTFailure(subject, service);
  }

  protected static void handleRESTFailure(
      SubjectEnrollmentReference subject, RemoteAccessService service) {
  ...
  public static void handleBiographicInsertFailure(BiographicProfileRecord biographicRecord) {
    ...
    String pendingRecordFile =
        FileUtils.expandConfigFile("test/errors/pendingRecords", id, FileUtils.EXT_DAT);
    try {
  ...
  protected void handleVerificationFailure(
      SubjectEnrollmentReference enrollmentRef, VerifyResponse verifyResponse) {
    MatchResult matchResult = verifyResponse.getMatchResult();
    double score = matchResult.getMatchScore();
    // etc
  }

  protected void handleUpdateFailure(
      SubjectEnrollmentReference enrollmentRef, UpdateResponse updateResponse) {
  ...
  protected void handleMatchResults(
      BiographicProfileRecord biographicRecord, MatchEngineResponse response) {
  ...
  protected void examineMatchResult(
      String subjectManifest,
      String matchManifest,
      ImageBlob subjectPortrait,
      ImageBlob matchPortrait) {}

  protected void displayParameterStatus(ParameterStatus parameterStatus) {
    String message =
        String.format(
            "Status %d:%s", parameterStatus.getStatusCode(), parameterStatus.getErrorMessage());
    println(message);
  }
}
