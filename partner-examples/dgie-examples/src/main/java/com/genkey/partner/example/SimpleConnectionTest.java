 ...
 *
 * <p>
 * Initialization is performed through the DGIEServiceModule and
 * ABISServiceModule facades which are initialized with host-name, port and the
 * default domain name that should be applied for system level scoping of
 * biographic-identifier names
 *
 * <p>
 * The examples show how to test for availability and how to interrogate
...
public class SimpleConnectionTest extends PartnerExample {
	...
	/**
	 * This performs an initialization of services and then performs a remote
	 * restart on the ABIS services that run on the server.
	 *
	 * <p>
	 * Note this is a test action only and will not work unless the server is
	 * configured to run in test mode.
	 */
	@Test
	...
	/**
	 * Shows the standard pattern for initializing the SDK and accessing REST
	 * services
	 */
	@Test
	public void testStandardConnection() {
		...
		if (!abisService.testAvailable()) {
			...
			// If connection is successful the getLastErrorMessage returns the connection
			// status
	...
	public void testLegacyMatchingService() {
		...
		LegacyMatchingService legacyService = DGIEServiceModule.getLegacyService();
		LegacyMatchingService legacyService2 = RestServices.getInstance()
				.accessServiceInstance(LegacyMatchingService.class);
	...
	private static void checkRemoteServiceConnection(RemoteAccessService service) {
		...
		if (!service.testAvailable()) {
			...
			PrintMessage("Error connecting to service @" + hostName + ":" + port + " with Status=" + statusCode + " :"
					+ errorMessage);
		} else {
