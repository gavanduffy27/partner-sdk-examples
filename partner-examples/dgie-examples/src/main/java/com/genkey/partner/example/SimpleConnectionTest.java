 ...
 *
 * <p>Initialization is performed through the DGIEServiceModule and ABISServiceModule facades which
 ...
 *
 * <p>The examples show how to test for availability and how to interrogate parameter from the
 * service
 *
 * @author Gavan
 */
public class SimpleConnectionTest extends PartnerExample {
  ...
  /**
   * This performs an initialization of services and then performs a remote restart on the ABIS
   * services that run on the server.
   *
   * <p>Note this is a test action only and will not work unless the server is configured to run in
   * test mode.
   */
  @Test
  public void testABISReset() {
    DGIEServiceModule.initCoreServices(
        getPrimaryHost(), getServicePort(), PartnerExample.getPartnerDomainName());
  ...
  public void testStandardConnection() {
    ...
    LegacyMatchingService service =
        RestServices.getInstance().accessServiceInstance(LegacyMatchingService.class);
    ...
    if (!abisService.testAvailable()) {
      ...
      PrintMessage(
          "Failed access to ABIS service with HTTP status code "
              + status
              + " error "
              + errorMessage);
    } else {
  ...
  public void testLegacyMatchingService() {
    ...
    LegacyMatchingService legacyService = DGIEServiceModule.getLegacyService();
    LegacyMatchingService legacyService2 =
        RestServices.getInstance().accessServiceInstance(LegacyMatchingService.class);
