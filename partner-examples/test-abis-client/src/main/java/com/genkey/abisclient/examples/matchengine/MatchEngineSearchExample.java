...
public class MatchEngineSearchExample extends MatchEngineExample {
  ...
  */
  ...
  public void simpleCrashTest() {
    ...
    Subset subset =
        MatchEngineTestUtils.loadSubset(1, 4, MatchEngineTestUtils.DefaultFingers, 1, false);
    subset.clear();
  }
  ...
  public void simpleDetectionTest(int nSubjects, int nClones, boolean usePositionIndependent) {
    simpleDetectionTest(
        MatchEngineTestUtils.DefaultFingers, nSubjects, nClones, usePositionIndependent);
  }

  public void simpleDetectionTest(
      int[] fingers, int nSubjects, int nClones, boolean usePositionIndependent) {
    int firstSubject = 1;
    int[] testFingers = fingers; // MatchEngineTestUtils.DefaultFingers;
    LocalMatchEngine engine = new LocalMatchEngine();
    Subset subset =
        MatchEngineTestUtils.loadSubset(
            firstSubject, firstSubject + nSubjects - 1, fingers, 1, UseCache);
    ...
    LocalMatchEngine.setThresholdParameter(
        GKMatcherThresholdParameter.GKThresholdPrefilterTriggerSize, 1);
    ...
    for (int ix = 0; ix < nSubjects; ix++) {
      ...
      List<MatchResult> results1 =
          engine.findDuplicates(testSubject, 60, 3, usePositionIndependent);
  ...
  public void testLargeScaleMatch(int nClones) {
    ...
    LocalMatchEngine.setThresholdParameter(
        GKMatcherThresholdParameter.GKThresholdPrefilterTriggerSize, 20);

    // Default setting is that no more than 1% of subjects examined from first stage will be
    // promoted to subsequent matching
    LocalMatchEngine.setThresholdParameter(
        GKMatcherThresholdParameter.GKThresholdPrefilterEfficiency, 0.01f);

    // Absolute cap on the number of candidate subjects that are permitted to be examined on the
    // second stage
    LocalMatchEngine.setThresholdParameter(
        GKMatcherThresholdParameter.GKThresholdPrefilterMaxResults, 50);
