...
public class HajoReplayTest extends ExampleModule {
  ...
  private void waitThreadsStop(
      List<SoakTestThread> threads, long reportInterval, double maxMinutes) {
          ...
          if (intervalTimer.getDurationMillis() > nextInterval) {
            ...
            if (lapsedTimeMins > maxMinutes) {
              ...
              FormatUtils.printBanner(
                  "Thread notified for exit on timeout at " + lapsedTimeMins + " minutes", '$');
            } else {
              FormatUtils.printBanner(
                  thread.getName() + " Interval timer after " + lapsedTimeMins + " minutes", '#');
  ...
  public void biohashReplayExample(String dirName) {
    ...
    if (referenceList.size() > 0) {
      ...
      AnonymousFusionReference fusionReference =
          extractor.createFromReferences(referenceList, true, null);
      // printObject("FusionReference Summary", fusionReference);
      FormatUtils.printBanner(
          threadName
              + "::"
              + "FusionReference created for "
              + CollectionUtils.containerToString(
                  new ArrayIterator<Integer>(fusionReference.getFingers()), ",")
              + " for "
              + FileUtils.baseName(dirName));
