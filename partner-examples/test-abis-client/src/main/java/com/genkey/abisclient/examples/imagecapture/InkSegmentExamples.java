...
public class InkSegmentExamples extends ExampleModule {
  ...
  public void convertTestImages() {
    String path1 =
        SampleTestPath("sample2"); // FileUtils.expandConfigPath("replay/hajo/problem2/crash1");
    String path2 =
        SampleTestPath("sample3"); // FileUtils.expandConfigPath("replay/hajo/problem2/crash1");
  ...
  public void segmentInkExamples(int sourceResolution, boolean showSegments) {
    ...
    for (String imageName : imageNames) {
      ...
      // String exportFile = FileUtils.expandConfigFile(exportDirectory, baseName,
      // ImageData.FORMAT_WSQ);
  ...
  private static ImageData loadResampledImage(
      String imageName, int sourceResolution, int targetResolution) {
    try {
      ...
      // Easy constructor to generate from BufferedImage with specified source and target
      // resolutions
