...
//		printObject("BioHASH", biohash.printReferenceState());
		...
		List<ReferenceDataItem> references = loadVerificationReferences(testSubject, fingers, 2, version);
	...
	public void verificationFalureTestTest(AnonymousFusionReference biohash, int testSubject) {
		...
		// Short cut to capture and generate references
		List<ReferenceDataItem> referencesBad = loadVerificationReferences(testSubject, fingers, 2, badVersion);
		List<ReferenceDataItem> referencesGood = loadVerificationReferences(testSubject, fingers, 2, version);
...
//			e.printStackTrace();
	...
	private List<ReferenceDataItem> loadVerificationReferences(int testSubject, int[] fingers, int sampleIndex, int version) {
		...
		for(int ix=0; ix < imageFiles.size(); ix++) {
			...
			// Instantiate imageContext with version information and indication to use verification mode as true
