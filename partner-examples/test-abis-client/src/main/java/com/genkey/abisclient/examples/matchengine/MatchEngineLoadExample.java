...
public class MatchEngineLoadExample extends MatchEngineExample{
    ...
    public static void easyLoadMatchEngine(long firstSubject, long lastSubject, int[] fingers, int sampleIndex, boolean useCache)
    {
        Subset subset = MatchEngineTestUtils.loadSubset(firstSubject, lastSubject, fingers, sampleIndex, useCache);
    ...
    public static void testMultiFileLoad()
    {
        ...
        PrintMessage("Database read back from file " + fileName + " with " + readBack1.size() + " subjects");
        readBack1.clear();

        // Recover from file with ImportFromDisk
        Subset readBack2 = new Subset();
        readBack2.importFromDisk(dirNameSplit);
        PrintMessage("Database read back from directory " + dirNameSplit + " with " + readBack2.size() + " subjects");
        readBack2.clear();
    }
