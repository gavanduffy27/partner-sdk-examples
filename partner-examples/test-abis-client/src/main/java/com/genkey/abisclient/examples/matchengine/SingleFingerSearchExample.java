...
public class SingleFingerSearchExample extends MatchEngineExample {
    ...
    ///     followed by an iterated match on a second set that is generated based on sample(2) for same
    ...
    /// <param name="useCache">if true then allow the subset to be saved and accessed from file-based cache</param>
    /// <returns></returns>
    public Subset doSimulatedEnrolment(LocalMatchEngine engine, long start, long  last, int [] fingers, int sampleIndex,boolean useCache)
    {
        ...
        // iterate through subjects, matching first against the match-engine and addint to subset if not present
