...
public abstract class TransportExampleModule  extends ExampleModule{
	 ...
	 * @return
	 */
	protected FingerEnrollmentReference generateFingerBlob(int subjectId, int finger, int sampleIndex, boolean useEncodedImage, boolean autoExtract) {
	...
	/**
	 * Creates a new instance of a subject enrollment reference for specified subject, fingers and samples.
	 * This constructs the instance by iterated calls to  {@link #generateFingerBlob(int, int, int, boolean, boolean)}
	 * which are then simply added to the container.
	 * @param subjectId
	 * @param fingers
	 * @param samples
	 ...
	 * @return
	 */
	public SubjectEnrollmentReference generateSubjectBlob(int subjectId, int[] fingers, int[] samples, boolean useEncodedImage,
			boolean autoExtract) {
		...
		for(int finger : fingers) {
			for(int sample : samples) {
				FingerEnrollmentReference fingerReference = generateFingerBlob(subjectId, finger, sample, useEncodedImage, autoExtract);
	...
	/**
	 * Performs a simulated breadth first incremental enrollment where the enrollment data for a subject is extended
	 * with an additional sample for the specified set of fingers. 
	 ...
	 * @param autoExtract
	 */
	public void enrollSubjectBlob(SubjectEnrollmentReference subjectReference, int[] fingers, int sampleIndex,
			boolean useEncodedImage, boolean autoExtract) 
	{
		int subjectId = Integer.valueOf(subjectReference.getSubjectID());
		for(int finger : fingers) {
			FingerEnrollmentReference fingerReference = generateFingerBlob(subjectId, finger, sampleIndex, useEncodedImage, autoExtract);
			subjectReference.add(fingerReference);
		}
	...
	public void exportXMLFile(String moduleName, SubjectEnrollmentReference subjectReference, GKXmlSchema schema) {
		
		// Construct the name of the test file
		String fileBaseName = StringUtils.concat(subjectReference.getSubjectID(), 
	...
	public void subjectSerializeTest(SubjectEnrollmentReference sourceReference) {
	  ...
	  public void compareXML(SubjectEnrollmentReference subjectReference1, SubjectEnrollmentReference subjectReference2)
      {
