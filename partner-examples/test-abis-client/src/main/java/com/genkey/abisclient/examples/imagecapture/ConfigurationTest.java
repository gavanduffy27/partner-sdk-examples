...
public class ConfigurationTest extends ExampleModule {
  ...
  public void testExceptionCatch() {
    try {
      ...
      throw new ABISClientException(
          "External error description", (int) ErrorStatusCodes.BORDER_INFRINGEMENT_ERROR);
  ...
  public void enrollmentSettingTest() {
    ...
    int emulationDomain =
        (int) ImageContextSDK.getThreshold(GKThresholdParameter.ServerEmulationDomain);
    ...
    int fingerSHortFall =
        (int) ImageContextSDK.getThreshold(GKThresholdParameter.FingerShortFallMin);
