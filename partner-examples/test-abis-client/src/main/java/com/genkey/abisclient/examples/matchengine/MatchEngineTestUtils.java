...
public class MatchEngineTestUtils {
  static String DBPath() {
    return DBPath(ABISClientLibrary.getActiveVersion());
  }

  static String DBPath(int version) {
    return "dbj_" + version + "/subsets";
  }
  ...
  public static Subset loadSubset(
      long start, long last, int[] fingers, int sampleIndex, boolean useCache) {
    return useCache
        ? loadCachedSubset(start, last, fingers, sampleIndex)
        : enrollSubset(start, last, fingers, sampleIndex);
  }
  ...
  public static void extendSubsetWithClones(
      Subset subset, Subject subject, int startId, int nSubjects) {
