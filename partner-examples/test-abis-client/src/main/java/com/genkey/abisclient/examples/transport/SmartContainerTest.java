...
/**
 * Use of SubjectEnrollmentReference as smart container configured by quality threshold and target
 * sample count and used as a control object for data driven enrollment.
 *
 * @author Gavan
 */
public class SmartContainerTest extends TransportExampleModule {
  ...
  private void incrementalEnrolmentExample(
      int testSubject, int[] fingers, int qualityThreshold, int targetSampleCount) {
    ...
    // Start the enrollment terminating when the Subject Reference is complete or if we have no more
    // samples available
  ...
  private void performCheckedEnrollment(
      SubjectEnrollmentReference subjectReference,
      int shift,
      int startSample,
    ...
    while (!subjectReference.isComplete() && sampleIndex <= maxSamples) {
      ...
      for (int ix = 0; ix < fingersRequired.length; ix++) {
        ...
        FingerEnrollmentReference fingerReference =
            generateFingerBlob(subjectId, finger, sampleIndex, true, false);
        ...
        if (!subjectReference.addChecked(fingerReference)) {
          ...
          printHeader(
              "Add checked failure for finger " + finger + " ==> " + fingerReference.getFingerID());

          // Access state information
          ReferenceConsistencyState matchState =
              subjectReference.checkReferenceConsistent(fingerReference);
          ...
          ReferenceMatchValues matchValues =
              subjectReference.checkReferenceMatchValues(fingerReference);
          ...
          if (!consistent) {
            ...
            printResult(
                "Sample size/ MatchFinger count",
                sampleMatchScores.size() + "/" + fingerScoreMap.size());
