...
import com.genkey.abisclient.Enums.GKQualityRank;
...
//		doNullTest();
    ...
    public void simpleSingleFingerTest()
    {
        ...
        SubjectEnrollmentReference enrollmentInfo = new SubjectEnrollmentReference(String.valueOf(subjectId));
        ...
        for(int finger : fingers)
        {
            ...
            while (!captured)
            {
                ...
                if (context.isBlocked())
                {
                    ...
                    // Display feedback
                    PrintMessage("Image is rejected with status code " + status + " and error description " + errorDescription);
                ...
                else if (ErrorStatusCodes.isWarning(status))
                {
                    ...
                    PrintMessage("Warning on image with status " + status + " description " + warningDescription);
                }
                ...
                double qualityThreshold = ImageContextSDK.
                                	getThreshold(GKThresholdParameter.MinutiaQualityThreshold);
