...
public class DuplicateDetectionTest extends ExampleModule {
  ...
  public void duplicateDetectionTest() {
    ...
    for (int ix = 0; ix < fingers.length; ix++) {
      ...
      if (isDuplicate) {
        printMessage(
            "Unexpected match of reference["
                + ix
                + "] with reference["
                + matchDetails.getIndex()
                + "] with score = "
    ...
    for (int ix = 0; ix < fingers.length; ix++) {
      ...
      boolean isDuplicate =
          ImageContextSDK.isDuplicate(reference, referenceList, 0.2f, matchDetails);
      if (isDuplicate) {
        printMessage(
            "Expected match of reference["
                + ix
                + "] with reference["
                + matchDetails.getIndex()
                + "] with score = "
        ...
        printMessage(
            "Unexpected match failure of reference["
                + ix
                + "] with reference["
                + matchDetails.getIndex()
                + "] with best score = "
