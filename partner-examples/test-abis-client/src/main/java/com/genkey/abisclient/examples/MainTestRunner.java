...
public class MainTestRunner extends BaseTestRunner {
  ...
  private static void initializeLibrary(ArgumentMap argMap) {
    ...
    if (!ABISClientLibrary.isInitialized()) {
       ...
       * String configDirectory = argMap.getArgument(ARG_CONFIG_DIRECTORY); boolean
       * defaultEmbedded = AndroidPlatform.isAndroidPlatform(); boolean useEmbedded =
       * argMap.getArgumentBoolean(ARG_EMBEDDED, defaultEmbedded); boolean useLegacy =
       * argMap.getArgumentBoolean(ARG_USE_LEGACY, false); boolean enableBC =
       * argMap.getArgumentBoolean(ARG_ENABLE_BC, false);
       *
       * int securityDomain = argMap.getArgumentInt(ARG_SECURITY_DOMAIN, -1); int
       * bskCode = argMap.getArgumentInt(ARG_SERVER_DOMAIN, -1); //
      ...
      /*
       * if (settings.is) { ABISClientLibrary.initAndroid(hardwareId, false); } else {
       ...
       *
       * boolean useLog10Far = VerificationSDK.isUseFAR10ScoreScheme(); if (!
       * useLog10Far ) { AnonymousReferenceMatcher.setUseFAR10ScoreScheme(true);
       * useLog10Far = AnonymousReferenceMatcher.isUseFAR10ScoreScheme(); }
       *
       * ImageContextSDK.setThreshold(GKThresholdParameter.ServerEmulationDomain,
       * bskCode); if (!useLegacy && enableBC) {
     ...
     * if(securityDomain != -1) { VerificationSDK.setSecurityDomain(securityDomain);
     * } boolean useStandardConfig =
     * argMap.getArgumentBoolean(ARG_USE_STANDARD_CONFIG, true);
  ...
  private static void initializeGUI(ArgumentMap argMap) {
    ...
    if (messageHandlerClass != null) {
      try {
        ...
        throw new GKRuntimeException(
            "Unable to instantiate message handler " + messageHandlerClass);
      }
    } else {
  ...
  public static void miscTests() {
     ...
     * decodingTest.runTestExamples(); VerifyTestClass verifyTestClass = new
     * VerifyTestClass(); verifyTestClass.runSelectedExamples();
     */
  ...
  public static void runSelectedTests() {
     ...
     * fingerExample.runTestExamples(); SmartContainerTest containerTest = new
     * SmartContainerTest(); containerTest.runTestExamples();
     */
  }
}
