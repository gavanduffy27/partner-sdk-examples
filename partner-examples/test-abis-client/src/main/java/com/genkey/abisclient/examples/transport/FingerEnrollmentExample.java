...
public class FingerEnrollmentExample extends TransportExampleModule  {
	...
	private void doFingerEnrollmentExample(int subjectId, int[] fingers, boolean useEncodedImage, boolean autoExtract) {
        for(int finger : fingers) {
            FingerEnrollmentReference fingerBlob = generateFingerBlob(subjectId, finger, 1, useEncodedImage, autoExtract);
	...
	private void transportTestFingerBlob(FingerEnrollmentReference xmitReference, boolean useCompact) {
        ...
        if (recvReference.hasImage())
        {
            ...
            // It would be unusual not to ave image but it is supported through SubjectEnrollmentReference to dispose images
        ...
        if (recvReference.hasReference())
        {
            ...
            for(String algorithm : algorithms) {
                ...
                // Can now access correct name from XML - at least this is the name used within binary encoding 
                // Assume this is also correct for XML
                String abis3TemplateName = StructuredTemplate.asABIS3TemplateName(algorithm);
                printMessage("Processing ABIS3 template " + abis3TemplateName + " of size " + abis3Encoding.length + " from raw encoding of size " + rawTemplate.length);
            }
        ...
        if (recvReference.hasImage()) {
        	...
        	FormatUtils.printResult("Encoding reduction before/after", encoding1.length + "/" + encoding2.length);
        }
	}	
